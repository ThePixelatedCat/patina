...
use crate::{
    ...
    parser::ast::{Ast, Binding, BindingS, Bop, Expr, ExprS, Item, Unop}, typecheck::types::{Type, TypeId},
};
...
impl TypeChecker {
    pub fn unify(&mut self, a: Type, b: Type) -> Result<(), TypeError> {
        ...
        match (a, b) {
            (Type::Var(id_l), Type::Var(id_r)) => { self.table.insert(id_l, Type::Var(id_r)); },
            (Type::Var(_), Type::Named(_, type_as)) => todo!(),
            (Type::Named(_, type_as), Type::Var(_)) => todo!(),
            (Type::Named(n_l, type_args_l), Type::Named(n_r, type_args_r)) => if n_l == n_r {

            } else {
                return Err(TypeError::MismatchedTypes(n_l, n_r))
            },
        }
